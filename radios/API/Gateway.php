   ...
   public function token()
{
    ...
    return $this->callAPI('POST', self::TokenURL, [
        ...
        'callbackURL' => $this->callBackURL . 'invoice=' . $this->invoiceId,
        'paymentId' => 0,
        'additionalData' => '',
    ]);
}
    ...
    {
        $result = $this->callAPI('POST', self::VerifyURL, [
            'merchantConfigurationId' => $this->merchantConfigID,
            'payGateTranId' => $transId
        ]);
        
        // اضافه کردن لاگ دقیق
        file_put_contents(__DIR__ . '/verify_raw_log.txt', 
            date('Y-m-d H:i:s') . PHP_EOL .
            "TransId: " . $transId . PHP_EOL .
            "Raw Result: " . print_r($result, true) . PHP_EOL .
            "Raw Content String: [" . ($result['content'] ?? 'NULL') . "]" . PHP_EOL .
            str_repeat('-', 80) . PHP_EOL,
            FILE_APPEND
        );
        
        // اگر محتوا خالی بود، از روش دیگری استفاده کن
        if (empty($result['content'])) {
            // شاید تراکنش قبلاً تایید شده است
            // برای آسان پرداخت، کد 2 به معنای قبلاً تایید شده است
            return ['code' => $result['code'], 'content' => json_encode(['resultCode' => 2])];
        }
    ...
    public static function redirect($token, $mobile = null)
    {
        ...
        echo '<script language="javascript" type="text/javascript">
            ...
            postRefId(' . $token . ',' . ($mobile ? "'$mobile'" : 'null') . ')
        </script>';
        echo '</body>';
        echo '</html>';
    }
